...
CREATE TABLE `cli_categories` (
  ...
  `has_child` tinyint(1) DEFAULT 0 COMMENT '是否有子节点 0=否 1=是',
...
java -jar jeecg-codegen-cli.jar \
  --ddl /path/to/cli_categories.sql \
  --spec-out /path/to/specs/cli_categories.yaml \
...
java -jar jeecg-codegen-cli.jar \
  --input /path/to/specs/cli_categories.yaml \
  --output /abs/path/jeecg-boot/jeecg-module-system/jeecg-system-biz \
  --frontend-root /abs/path/jeecg-boot/ant-design-vue-jeecg/src/views \
  --dry-run
